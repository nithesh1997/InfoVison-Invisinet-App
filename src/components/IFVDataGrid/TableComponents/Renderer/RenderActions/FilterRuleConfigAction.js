...
export const FilterRuleConfigAction = ({
  actionPayloadState,
  editOnPopup,
  inputRef,
  isEnabled = () => true,
  ...
  const handleClick = (event) => {
    ...
    setTimeout(() => {
      setStartAsync((prev) => ({ ...prev, isFilterRulesConfig: true }));
    }, 0);
  };
  ...
  React.useEffect(() => {
    if (rowActionState.inProgress && startAsync.isFilterRulesConfig) {
      setRowActionState((prev) => ({ ...prev, inProgress: true }));
  ...
  React.useEffect(() => {
    if (
      ...
      startAsync.isFilterRulesConfig === true
    ) {
      setStartAsync((prev) => ({ ...prev, isFilterRulesConfig: false }));
  ...
  return (
    <Fragment>
      <ToolTip
        title={t(
          "page.Endpoint.Configure.ellipsisMenu.tooltipConfigureFilterRles",
        )}
      >
        <ACTION_BUTTON
          id={id}
